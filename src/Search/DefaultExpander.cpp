...
bool DefaultExpander::IsMoreCnot(SearchNode* node) {
    ...
    for(int i=0;i<node->remainGate.size();i++){
        ...
        if (this->env->gate_info[gateid].control!=-1){
            if(node->environment->coupling_graph_matrix[node->p2lMapping[this->env->gate_info[gateid].control]][node->p2lMapping[this->env->gate_info[gateid].target]]!=1){
                moreCx=true;
                return moreCx;
            }
        }
...
void DefaultExpander::expandWithoutSwap(SearchNode *node) {
    ...
    //找到ready的门然后执行，这里其实就是只有第一层的门，只要在最初的时候满足qubit state的要求
    ...
    for (set<int>::iterator iter = frontLayerGate.begin(); iter != frontLayerGate.end(); ++iter){
        ...
        if (nowGate.control == -1) {
            if (qubitState[nowGate.target] == 0) {
                newReadyGate.push_back(*iter);
            }
        }
        else{
            if (qubitState[nowGate.target] == 0&&qubitState[nowGate.control] == 0) {
                newReadyGate.push_back(*iter);
            }
        }
    }
    ...
    while(remainGate.size()>0){
        ...
        for(int i=0;i<newReadyGate.size();i++){
            ScheduledGate Sg;
            if(this->env->gate_info[newReadyGate[i]].control==-1){
                Sg.gateName=="cx";
                Sg.controlQubit=node->p2lMapping[this->env->gate_info[newReadyGate[i]].control];
            }
            else{
                Sg.controlQubit=-1;
            }
            Sg.controlQubit=node->p2lMapping[this->env->gate_info[newReadyGate[i]].target];
            Sg.gateID=newReadyGate[i];
        ...
        for(int i=0;i<remainGate.size();i++){
            ...
            for(int j=0;j<newReadyGate.size();j++){
                if(newReadyGate[j]==remainGate[i]){
                    flag=false;
                    break;
                }
            }
...
vector<vector<vector<int>>> DefaultExpander::  SwapCom2(vector<int> qubitState,vector<int> l2pmapping,int qubit_num,set<int> cnot_qubit) {
    vector<vector<int>> possibleSwap;
    for(int i=0;i<this->env->coupling_graph_list.size();i++){
        int a=this->env->coupling_graph_list[i][0];
        int b=this->env->coupling_graph_list[i][1];
...
bool DefaultExpander::ExpandWithoutCnotCheck(DefaultQueue *nodes, SearchNode *node, HashFilter_TOQM* filter_T) {
    int countNum=0;
    int cycleNum=0;
//    cout<<"in expand without check"<<endl;
...
//        cout<<endl;
        ...
        if(0){
            ...
            cout<<"         ////        logical qubit on physical";
...
//                cout<<"target is :"<<nowGate.target<<" control is "<<nowGate.control<<endl;
            ...
            //生成新的ActionPath
//            cout<<"check the action path"<<endl;
            ActionPath thisAction;
            thisAction.actions=thisTimeSchduledGate;
            bool IsPattern;
//            cout<<"node->getReadyGateSize()"<<endl;
            if(node->getReadyGateSize()==newReadyGate.size()){
                IsPattern=false;
            }
            else{
            ...
            vector<ActionPath> path=node->actionPath;
            path.push_back(thisAction);
//            cout<<"qubit state update and time"<<endl;
...
bool DefaultExpander::ExpandWithCnotCheck(DefaultQueue *nodes, SearchNode *node, HashFilter_TOQM* filter_T) {
    int countNum=0;
    int cycleNum=0;
//    cout<<"in expand without check"<<endl;
    //dead or not 如果这个结点已经死了，那么意味着队列里有比这个结点好的结点都使用过了
